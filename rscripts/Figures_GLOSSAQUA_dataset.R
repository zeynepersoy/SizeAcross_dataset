...
coord.A <- tibble(
  StudyID = Size_Sample.map$StudyID, SiteID = Size_Sample.map$SiteID,
...
Aquatic_avg.coordinates <- Size_Sample.map %>%
  group_by(GeographicalTerritory2) %>%
...
Source_Sample.Aquatic <- Source_Sample.Aquatic %>%
  mutate(
    SpeciesType = as.character(SpeciesType),
...
Size_Sample <- full_join(Sample, Size, by = c("StudyID", "SiteID")) %>%
  ...
  mutate(across(c(Slope, Intercept, Linearity), ~ as.numeric(gsub(",", ".", .x))))
...
Slope <- ggplot(Size_Sample, aes(x = fct_infreq(SizeSpectrumMethod), y = Slope)) +
  ...
  geom_jitter(aes(x = as.numeric(fct_infreq(SizeSpectrumMethod)) + 0.3), width = 0.1, colour = "grey70", alpha = 0.3) +
...
Intercept <- ggplot(Size_Sample.Intercept, aes(x = fct_infreq(SizeSpectrumMethod), y = Intercept)) +
  ...
  geom_jitter(aes(x = as.numeric(fct_infreq(SizeSpectrumMethod)) + 0.25), width = 0.1, colour = "grey70", alpha = 0.3) +
