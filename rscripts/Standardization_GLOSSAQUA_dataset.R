...
for (i in 1:length) {
  ...
  if (site$SizeSpectrumMethod == "NASS") { # value is lambda
    Size_Sample$Slope_mod[count] <- as.numeric(site$Slope)
  } else if (site$SizeSpectrumMethod == "NBSS") { # value is lambda - 1
    Size_Sample$Slope_mod[count] <- as.numeric(site$Slope) - 1
  } else if (site$SizeSpectrumMethod == "ASS") { # value is lambda - 1
    Size_Sample$Slope_mod[count] <- as.numeric(site$Slope) - 1
  } else if (site$SizeSpectrumMethod == "BSS") { # value is lambda - 2
    Size_Sample$Slope_mod[count] <- as.numeric(site$Slope) - 2
  } else if (site$SizeSpectrumMethod == "MLE") { # value is lambda (Edwards et al. 2016)
    Size_Sample$Slope_mod[count] <- as.numeric(site$Slope)
